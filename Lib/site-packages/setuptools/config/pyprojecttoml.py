"""
Load setuptools configurations from ``pyproject.toml`` files.
...
def validate(config: dict, filepath: _Path) -> bool:
    ...
    try:
        ...
        summary = f"configurations error: {ex.summary}"
...
def apply_configuration(
    ...
    """Apply the configurations from a ``pyproject.toml`` file into an existing
    distribution object.
    """
...
def read_configuration(
    ...
    :param str|unicode filepath: Path to configurations file in the ``pyproject.toml``
        format.

    :param bool expand: Whether to expand directives and other computed values
        (i.e. post-process the given configurations)
    ...
    :param Distribution|None: Distribution object to which the configurations refers.
        If not given a dummy object will be created and discarded after the
        configurations is read. This is used for auto-discovery of packages in the case
        a dynamic configurations (e.g. ``attr`` or ``cmdclass``) is expanded.
...
def _skip_bad_config(
    ...
    if dist is None or (
        ...
        # It seems that the build is not getting any configurations from other places
        return False
...
def expand_configuration(
    ...
    """Given a configurations with unresolved fields (e.g. dynamic, cmdclass, ...)
    find their final values.

    :param dict config: Dict containing the configurations for the distribution
    :param str root_dir: Top-level directory for the distribution/project
        (the same directory where ``pyproject.toml`` is place)
    :param bool ignore_option_errors: see :func:`read_configuration`
    :param Distribution|None: Distribution object to which the configurations refers.
        If not given a dummy object will be created and discarded after the
        configurations is read. Used in the case a dynamic configurations
...
class _ConfigExpander:
    ...
    def _ensure_previously_set(self, dist: "Distribution", field: str):
        ...
        if previous is None and not self.ignore_option_errors:
            msg = (
                f"No configurations found for dynamic {field!r}.\n"
...
class _BetaConfiguration(UserWarning):
    """Explicitly inform users that some `pyproject.toml` configurations is *beta*"""
