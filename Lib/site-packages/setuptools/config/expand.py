"""Utility functions to expand configurations directives or special values
(such glob patterns).

We can split the process of interpreting configurations files into 2 steps:
...
2. The expansion (or post-processing) of these values according to the
   semantics ``setuptools`` assign to them (for example a configurations field
   with the ``file:`` directive should be expanded from a list of file paths to
   a single string with the contents of those files concatenated)

This module focus on the second step, and therefore allow sharing the expansion
functions among several configurations file formats.
...
def _find_module(
    ...
    considering the ``package_dir`` in the build configurations and ``root_dir``
    """
...
def find_packages(
    ...
    **kwargs
    ...
    similar data structure to the ``package_dir`` configurations parameter add fill-in
...
class EnsurePackagesDiscovered:
    ...
    the configurations. However, it is better to postpone calling package discovery as
